...
export function ActivityHeatmap({ data }: ActivityHeatmapProps) {
  ...
  const getLevelStyle = (level: number) => {
    switch (level) {
      ...
      case 1:
        return 'bg-teal-900/40 border-teal-800/50'
      case 2:
        return 'bg-teal-700/50 border-teal-600/50'
      case 3:
        return 'bg-teal-500/60 border-teal-400/50'
      case 4:
        return 'bg-teal-400 border-teal-300'
      default:
        return 'bg-gray-800/50 border-gray-700/30'
    }
  }
  ...
  return (
    <div className="bg-black/20 border border-white/5 rounded-2xl p-6 backdrop-blur-xl relative overflow-visible">
      ...
      <div className="relative z-10">
        ...
        {/* Month Labels */}
        <div
          className="flex relative mt-2 h-4 text-[10px] text-gray-500 font-medium overflow-hidden pointer-events-none"
          aria-hidden="true"
        >
      ...
      <div className="mt-4 flex items-center justify-end gap-2 text-[10px] text-gray-500 font-medium uppercase tracking-wider relative z-10">
        <span>Less</span>
        <div
          className={`w-3 h-3 rounded-[2px] border ${getLevelStyle(0)}`}
        ></div>
        <div
          className={`w-3 h-3 rounded-[2px] border ${getLevelStyle(2)}`}
        ></div>
        <div
          className={`w-3 h-3 rounded-[2px] border ${getLevelStyle(4)}`}
        ></div>
        <span>More</span>
      </div>
    </div>
