...
export const AuthInput = forwardRef<HTMLInputElement, AuthInputProps>(
  (
    { label, error, checkStatus, passwordRequirements, className, ...props },
    ref
  ) => {
...
function RequirementItem({ met, text }: { met: boolean; text: string }) {
  return (
    <div
      className={`flex items-center gap-2 text-xs transition-colors duration-200 ${met ? 'text-green-400' : 'text-gray-500'}`}
    >
      <div
        className={`w-1 h-1 rounded-full ${met ? 'bg-green-400' : 'bg-gray-600'}`}
      />
      <span>{text}</span>
    </div>
  )
