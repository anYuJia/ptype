...
function Select<T extends string>({ value, onChange, options, disabled = false, placeholder }: SelectProps<T>) {
    ...
    return (
        <Dropdown
            ...
            trigger={
                ...
                `}>
                    <span className={`font-medium tracking-wide ${disabled ? 'text-gray-600' : 'text-gray-200 group-hover:text-teal-400'}`}>
                        {selectedLabel}
                    </span>
                    <svg
...
export function BattleLobby({ onCreate, onJoin, onRefresh, rooms, error }: BattleLobbyProps) {
    ...
    const [textSource, setTextSource] = useState<'english' | 'chinese' | 'coder' | 'custom'>('english');
    const [chineseStyle, setChineseStyle] = useState<ChineseStyle>(DEFAULT_CHINESE_STYLE);
    const [programmingLanguage, setProgrammingLanguage] = useState<ProgrammingLanguage>(DEFAULT_PROGRAMMING_LANGUAGE);
            ...
            <AnimatePresence>
                {showCreateModal && (
                    ...
                    >
                        ...
                        >
                            <h3 className="text-2xl font-bold text-teal-400 mb-6">{t('modal.title')}</h3>

                            <div className="space-y-6">
                                {/* Mode Selection */}
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-gray-400">{t('modal.mode')}</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-gray-400">{t('modal.difficulty')}</label>
                                ...
                                {mode === 'time' && (
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-gray-400">{t('modal.timeLimit')}</label>
                                        <div className="flex gap-3">
                                            {[30, 60, 120].map(tVal => (
                                                <button
                                                    key={tVal}
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-gray-400">{t('modal.textSource')}</label>
                                            ...
                                            {textSource === 'coder' && (
                                                <Select
                                                    ...
                                                    options={PROGRAMMING_LANGUAGE_OPTIONS.map(lang => ({
                                                        value: lang,
                                                        label: t.has(`modal.languages.${lang}`) ? t(`modal.languages.${lang}`) : lang
                                                    }))}
                                                />
                                            )}
                                                ...
                                                isAuthenticated ? (
                                                    savedTexts.length > 0 ? (
                                                        <Select
                                                            ...
                                                            options={savedTexts.map(t => ({ value: t.content, label: t.title }))}
                ...
                <div className="lg:col-span-5 flex flex-col gap-6">
                    ...
                    >
                        ...
                        <div className="text-center">
                            <h3 className="text-xl font-bold text-gray-200">{t('createPrivate')}</h3>
                    ...
                    >
                        <div className="flex items-center gap-2 text-gray-400">
                            <div className="p-1.5 rounded-lg bg-blue-500/10 text-blue-400">
                                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11.536 19.464a3 3 0 01-.879.879l-4.343 4.343a3 3 0 01-4.242-4.242l4.343-4.343a3 3 0 01.879-.879l5.743 5.743A6 6 0 0115 7z" />
                                </svg>
                            </div>
                            <h3 className="text-sm font-bold uppercase tracking-wider">{t('joinById')}</h3>
                        </div>
                        ...
                        <AnimatePresence>
                            {rooms.length === 0 ? (
                                ...
                                >
                                    ...
                                    <p>{t('noRooms')}</p>
                                    <button onClick={() => setShowCreateModal(true)} className="text-teal-400 hover:underline">{t('beFirst')}</button>
                                </motion.div>
                            ) : (
                                rooms.map((room) => (
                                    <motion.div
                                        ...
                                        <div className="flex gap-4 items-center">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span className="font-bold text-gray-200">{t('hostRoom', { host: room.host })}</span>
                                                    <span className="text-xs px-2 py-0.5 rounded bg-gray-800 text-gray-400 border border-gray-700 uppercase">{room.mode}</span>
                                                    <span className="text-xs px-2 py-0.5 rounded bg-gray-800 text-gray-400 border border-gray-700 uppercase">{room.difficulty}</span>
                                                </div>
                                                <div className="text-xs text-gray-500 font-mono mt-1">ID: {room.id}</div>
                                            </div>
                                        </div>
