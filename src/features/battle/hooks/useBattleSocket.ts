...
export type BattleState = 'idle' | 'waiting' | 'starting' | 'playing' | 'finished';
...
export function useBattleSocket(username: string) {
    ...
    const createRoom = useCallback((config: Partial<BattleConfig>) => {
        connect();
    ...
    const updateProgress = useCallback((wpm: number, progress: number, correctChars: number) => {
