...
export function History() {
  ...
  const getSubModeLabel = (mode: string, subMode: string | null) => {
    ...
    if (mode === 'coder')
      return PROGRAMMING_LANGUAGE_LABELS[subMode as ProgrammingLanguage]
      ...
      <div className="w-full max-w-5xl mx-auto space-y-8 animate-pulse">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="bg-gray-900/50 border border-white/5 p-4 rounded-xl h-24"
            >
  ...
  return (
    ...
    >
        ...
        {[
          ...
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <span className="text-lg opacity-50 group-hover:opacity-100 transition-opacity grayscale group-hover:grayscale-0">
                {stat.icon}
              </span>
            </div>
      ...
      >
        <h3 className="text-sm font-medium text-gray-400 mb-4 absolute top-6 left-6 z-10">
          {t('cpmTrend')}
        </h3>
...
`}
                    ...
                    {item.subMode && (
                      <span
                        ...
                        title={getSubModeLabel(item.mode, item.subMode) || ''}
                      >
