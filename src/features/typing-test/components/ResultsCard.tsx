...
export function ResultsCard() {
  ...
  const { settings, wpm, cpm, lpm, accuracy, errors, cpmHistory, restart } =
    useTypingStore(
      useShallow((state) => ({
  ...
  const { mode, difficulty, chineseStyle, programmingLanguage, duration } =
    settings
        ...
        return [
          {
            value: wpm,
            label: tStats('wpm'),
            sublabel: tStats('sublabels.wordsPerMin'),
            color: 'text-teal-400',
          },
          {
            value: cpm,
            label: tStats('cpm'),
            sublabel: tStats('sublabels.charsPerMin'),
            color: 'text-cyan-400',
          },
        ]
      case 'chinese':
        ...
        return [
          {
            value: lpm,
            label: tStats('lpm'),
            sublabel: tStats('sublabels.linesPerMin'),
            color: 'text-teal-400',
          },
          {
            value: cpm,
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-8 md:gap-12 mb-8 flex-wrap">
        ...
        >
          <div className="text-5xl md:text-6xl font-extrabold text-emerald-400 tabular-nums">
            {accuracy}%
          </div>
          <div className="text-sm text-gray-400 mt-1">{tStats('acc')}</div>
          <div className="text-xs text-gray-500">
            {tStats('sublabels.accuracy')}
          </div>
        </motion.div>
        ...
        >
          <div className="text-5xl md:text-6xl font-extrabold text-red-400 tabular-nums">
            {errors}
          </div>
          <div className="text-sm text-gray-400 mt-1">{tStats('errors')}</div>
          <div className="text-xs text-gray-500">
            {tStats('sublabels.errorCount')}
          </div>
        </motion.div>
      </div>
