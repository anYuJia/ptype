...
export const useTypingStore = create<TypingState>((set, get) => ({
  ...
  initTest: (forceRegenerate?: boolean) => {
    ...
    const displayText = processTargetText(
      rawText,
      settings.mode,
      settings.englishOptions
    )
  ...
  appendText: () => {
    ...
    const processedNewChunk = processTargetText(
      newChunk,
      settings.mode,
      settings.englishOptions
    )
    const separatorDisplay = processTargetText(
      separator,
      settings.mode,
      settings.englishOptions
    )
  ...
  // 每秒计时器更新
  tick: () => {
    const { status, timeLeft, settings, startTime, correctChars, targetText } =
      get()
  ...
  updateSettings: (newSettings: Partial<TypingSettings>) => {
    ...
    if (
      (newSettings.mode && newSettings.mode !== settings.mode) ||
      (newSettings.difficulty &&
        newSettings.difficulty !== settings.difficulty) ||
      (newSettings.chineseStyle &&
        newSettings.chineseStyle !== settings.chineseStyle) ||
      (newSettings.programmingLanguage &&
        newSettings.programmingLanguage !== settings.programmingLanguage) ||
      ...
      const displayText = processTargetText(
        targetText,
        settings.mode,
        updatedSettings.englishOptions
      )
      set({
        displayText,
        lines: calculateAllLines(displayText, settings.mode),
