...
function getTextPool(
  ...
  if (mode === 'chinese') {
    ...
    const library =
      style === 'modern' ? chineseModernLibrary : chineseClassicalLibrary
  ...
  if (mode === 'coder') {
    ...
    type CodeLibraryType = {
      [K in 'easy' | 'medium' | 'hard']: { code: string }[]
    }
    ...
    console.warn(
      `Language ${lang} not yet migrated or not found, falling back to Python`
    )
