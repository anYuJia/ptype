...
export const bashLibrary: CodeLibrary = {
  ...
  hard: [
    ...
    {
      code: cleanCode(
        `rsync -avz --progress --exclude='node_modules' /source/ user@remote:/dest/`
      ),
      difficulty: 'hard',
      title: '远程同步排除目录',
      tags: ['同步', '网络'],
    ...
    {
      code: cleanCode(
        `for i in {1..10}; do echo "Processing $i"; sleep 1; done`
      ),
      difficulty: 'hard',
      title: 'For循环处理',
      tags: ['Shell脚本', '循环'],
    ...
    {
      code: cleanCode(
        `find . -name "*.jpg" -print0 | xargs -0 -P 4 -I {} convert {} -resize 50% {}.small`
      ),
      difficulty: 'hard',
      title: '并行处理图片',
      tags: ['xargs', '并行'],
